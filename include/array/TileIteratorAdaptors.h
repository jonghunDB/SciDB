...
namespace scidb
{
    ...
    template<typename TiledChunkIterator>
    void
    BufferedConstChunkIterator<TiledChunkIterator>::restart()
    {
        assert(! (_tiledChunkIterator->getMode() & TILE_MODE));
        clearCurrentLPosInTile();
        _tiledChunkIterator->restart();
    }
    ...
    public:
        ...
        Value const& getItem() override { return _itemChunkIterator->getItem(); }

        Coordinates const& getPosition() override { return _itemChunkIterator->getPosition(); }
        position_t getLogicalPosition();
        bool setPosition(Coordinates const& pos) override { return _itemChunkIterator->setPosition(pos); }
        bool setPosition(position_t pos) override;

        void operator ++() override { _itemChunkIterator->operator++(); }
        void restart()     override { _itemChunkIterator->restart(); }
        bool end()         override { return _itemChunkIterator->end(); }

        int getMode() const  override { return _itemChunkIterator->getMode(); }
        bool isEmpty() const override { return _itemChunkIterator->isEmpty(); }
        const scidb::ConstChunk& getChunk() override { return _itemChunkIterator->getChunk(); }
