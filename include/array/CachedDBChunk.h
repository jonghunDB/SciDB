...
#include <smgr/io/PersistentChunk.h>
...
namespace scidb {
class NewDBArray;
class NewDBArrayIterator;
class StorageAddress;
class Query;

/**
 * Chunk of NewDBArray managed by buffer cache
 */
class CachedDBChunk : public MemChunk
{
    friend class NewDBArrayIterator;
    friend class NewDBArray;
...
private:
    /* Note: CachedDBChunk objects are created by NewDBArrayIterators,
       but are deleted when the last chunk iterator that uses them
       unregisters. This allows the NewDBArrayIterator to "forget" about
...
public:
     ...
     */
    void const* getConstData() const override;
