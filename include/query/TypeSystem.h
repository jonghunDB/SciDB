...
/**
 *  Construct a value to represent the 'n' bytes of data that start at
 *  the address 'v'.  There is no data copy here, it's just a view:
 *  the pointed-to data is not owned by this object.
 */
inline Value::Value(void* v,size_t n,asView_t)
            : _code(MR_VIEW),
              _data(0)
{
    assert(implies(n!=0,v!=0));                          // Validate arguments
    ...
    if (large(n))                                        // Given large data?
    {
        _data = v;                                       // ...just point at it
    }
    else                                                 // It fits in '_data'
    {
        memcpy(&_data,v,n);                              // ...so just copy it
        _code = MR_DATUM;                                // ...and become datum
    }
...
inline Value::Value(const Value& v)
              ...
              _data(v._data)
 {
    ...
    if (large(_size))                                    // ...heap allocated?
    {
        ...
        if (v.isView())                                  // Source is a view?
        {
            _code = MR_DATUM;                            // ...ours is a copy
        }
    }
 ...
 *
 *  @note No memory is deallocated for large() Values.  (This seems wrong.)
 */
inline void Value::setNull(reason reason)
{
    ...
    if (isView())                                        // Not owner of *_data?
    {
        _data = nullptr;                                 // ...keep it that way
        _size = 0;
    }
...
inline t& Value::get()
{
    assert(!isTile());                                   // Must carry a datum or view
...
inline void Value::reset(const t& v)
{
    ...
    new(setSize<Value::IGNORE_DATA>(sizeof(t))) t(v);            // Resize, then copy
 ...
 *
 *  It is an error to call this method for a value that is carrying a 'tile'.
 *  Calling this method on a 'view' value transforms it into a 'datum'.
 */
template<Value::OldDataAction oldDataAction>
void* Value::setSize(size_t n)
{
    assert(!isTile());                                   // Once tile, always
    ...
    const bool isOldSizeLarge = large(_size);
    const bool isNewSizeLarge = large(n);
    const bool wasView = isView();

    _code = MR_DATUM;                                    // Now carrying datum

    // Size already correct?
    void*const oldData = isOldSizeLarge ? _data : &_data;
    if (!wasView && n == _size) {
        assert(consistent());
        return oldData;
    }
    ...
    // Allocate new buffer.
    void*const newData = isNewSizeLarge ? arena::mallocTLS(n) : &_data;
    if (isNewSizeLarge && newData == nullptr) {
        fail(SCIDB_LE_NO_MEMORY_FOR_VALUE);
    }

    // Copy original data.
    if (oldDataAction == KEEP_DATA && oldData != newData) {
        std::memcpy(newData, oldData, std::min(static_cast<size_t>(_size), n));
    }

    // Free old buffer.
    if (isOldSizeLarge && !wasView) {
        arena::freeTLS(oldData, _size);
    }

    // Set _data and _size.
    if (isNewSizeLarge) {
        _data = newData;
    } else if (oldDataAction == IGNORE_DATA) {
        _data = 0;
    }
...
inline void Value::setData(const void* v,size_t n)
{
    ...
    memcpy(setSize<Value::IGNORE_DATA>(n), v, n);               // Resize, then copy

    assert(consistent());                                // Check consistency
}

/**
 *  Assign to the value the 'n' bytes of storage starting at address
 *  'v', without copying large data.  The value may become a 'view'.
 *
 *  We can cast away the const-ness of 'v' because we know that any
 *  attempt to place data into this Value using public methods will
 *  turn it back into a 'datum'.
 */
inline void Value::setView(const void* v, size_t n)
{
    if (small(n))                           // Viewing small data?
    {
        setData(v, n);                      // ...just copy it normally
    }
    else
    {
        assert(!isTile());                  // Once tile, always
        ...
        reset();                            // Free old buffer if any
        _data = const_cast<void*>(v);       // Point at new data buffer
        _size = static_cast<size_type>(n);  // ...with proper length
        _code = MR_VIEW;                    // We don't own the data
...
inline bool Value::isString() const
{
    ...
    return isDatumOrView() && (_size==0 || getData<char>()[_size-1]==0);
}
...
inline Value& Value::operator=(const Value& v)
{
    ...
    {
        if (v.isDatumOrView() && !isTile())              // Possible shortcut?
        {
...
inline bool operator==(const Value& a,const Value& b)
{
    ...
    int32_t c1 = a.isView() ? Value::MR_DATUM : a._code; // 'view' and 'datum'
    int32_t c2 = b.isView() ? Value::MR_DATUM : b._code; // are equivalent here

    if (c1 != c2)                                        // Their codes differ?
    {
...
void Value::serialize(Archive& ar,unsigned int)
{
 ...
 /* We don't actually need to serialize the 'hasTile' flag any more, but still
...
inline void* Value::calloc(size_t n)
{
    ...
    void* p = arena::callocTLS(n,1);                        // Delegate to calloc
...
inline void* Value::malloc(size_t n)
{
    ...
    void* p = arena::mallocTLS(n);                          // Delegate to malloc
...
{
    if (large(_size) && !isView())                  // Has an owned allocation?
    {
        arena::freeTLS(_data, _size);               // ...so free it now
    }
}
