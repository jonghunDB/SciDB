...
/**
 * @brief Base class for logical expressions.
 */
...
/**
 * @brief A LogicalExpression that refers to an attribute.
 */
...
/**
 * A LogicalExpression that refers to a constant.
 */
...
public:
    Constant(const std::shared_ptr<ParsingContext>& parsingContext, const  Value& value,
        const  TypeId& type)
      : LogicalExpression(parsingContext), _value(value), _type(type)
    {
    }
    ...
    /**
     * @brief Retrieve a human-readable description.
     * Append a human-readable description of this onto str. Description takes up
     * one or more lines. Append indent spacer characters to the beginning of
     * each line. Terminate with newline.
     * @param[out] stream to write to
     * @param[in]  indent number of spacer characters to start every line with.
     */
    void toString(std::ostream &str, int indent = 0) const override;
...
/**
 * @brief A LogicalExpression that refers to a function.
 */
