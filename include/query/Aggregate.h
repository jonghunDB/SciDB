...
public:
    ...
    {
        return std::make_shared<BaseAggregate>(getName(), inputType, _resultType.typeId() == TID_VOID ? inputType : _resultType);
    }
...
public:
    ...
    AggregatePtr clone(Type const& inputType) const
    {
        return std::make_shared<BaseAggregateInitByFirst>(getName(), inputType, _resultType.typeId() == TID_VOID ? inputType : _resultType);
    }
 ...
 * @param distribution partitioning scheme for MemArray to hold inStates
