...
#include <array/Metadata.h>     // for ArrayID, VersionID
 ...
 *     of keys.
 *
 * KeyMeta is a template paramter to the DiskIndex template.  It encapsualtes
 * the interface that DiskIndex will use to manipulate keys. This is not a
 * formal class, just a description of the necessary attributes of any
 * class that will be used to instantiate the KeyMeta parameter.
 *
 * KeyMeta must inherit from DiskIndexKeyMetaBase.  KeyMeta must provide a
...
public:
    ...
    public:
        ...
        /**
         * Does iterator point to begining of index?
         */
        bool isBegin();
        ...
        /**
         * Return true if entry has a NULL value
         */
        bool isNullValue();
         ...
         */
        PointerRange<char const> getAuxMeta();
     ...
     * @throws BuffMgr::OutOfMemoryException if allocation fails
     */
     ...
     * @retrun an iterator pointing to the record with the requested key,
    ...
    using DiskIndexVisitor =
        boost::function<void(typename KeyMeta::Key const*, BufferMgr::BufferHandle&, PointerRange<const char>)>;
