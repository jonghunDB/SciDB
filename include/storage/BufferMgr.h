...
public:
    ...
    public:
        ...
        BufferKey(DataStore::DataStoreKey& dsk, off_t off, size_t sz, size_t alloc, size_t cSize);
    ...
    {
    public:
        static constexpr const uint64_t INVALID_SLOT = std::numeric_limits<BufferMgr::BlockPos>::max();
        static constexpr const uint64_t NULL_GENCOUNT = 0;
...
private:
    ...
    struct BlockHeader
    {
        ...
        /* Next free slot or next LRU */
        BlockPos next {MAX_BLOCK_POS};

        /* Prev LRU */
        BlockPos prev {MAX_BLOCK_POS};
    ...
    void _waitOnReserveQueue(size_t sz);
    ...
    void _markSlotAsFree(BlockPos slot);

    void _removeFromLru(BlockPos slot);

    void _updateLru(BlockPos slot);

    void _addToLru(BlockPos slot);
     ...
     */
    // clang-format off
    BlockHeaderList    _blockList;    // master list of blocks
    BlockPos           _lruBlock;     // head of lru list --- lru block (cold)
    BlockPos           _mruBlock;     // tail of lru list --- mru block (hot)
    BlockPos           _freeHead;     // free slots in the block list
