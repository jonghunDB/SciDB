...
public: // subclasses
     ...
     */
    typedef boost::function<void(std::weak_ptr<WorkQueue>&,
                      ...
                      INVALID_QUERY_ID)
    {
        ...
        {
            std::shared_ptr<OverflowException> ep =
                    std::make_shared<OverflowException>(_file.c_str(),
                                                        _function.c_str(),
                                                        _line);
...
private:
    ...
    typedef boost::function<void(std::weak_ptr<WorkQueue>&)> InternalWorkItem;
