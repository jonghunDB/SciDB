...
public:
    ...
    RWLock(): _nested(0), _readers(0), _noReaders(),
              _pendingWriters(0), _currentWriter(0), _noWriter()
    {
    }

    bool lockRead(ErrorChecker& errorChecker, perfTimeWait_t tw)
    {
    ...
    bool lockWrite(ErrorChecker& errorChecker, perfTimeWait_t tw)
    {
        ScopedMutexLock mutexLock(_mutex, PTW_SML_RWL);
