 ...
 *              a simple minded form of garbage collection, and, in conjunction
 ...
 *
 *  @see        https://paradigm4.atlassian.net/wiki/display/DEV/OperatorMemoryManagement
...
class Arena : noncopyable
{
 ...
 public:
    static std::shared_ptr<Arena> getArenaTLS() noexcept;
    static void setArenaTLS(const std::shared_ptr<Arena>&);

 private:
    static thread_local std::shared_ptr<Arena> _arenaTLS;
};

/// The class temporarily sets an arena in TLS (Thread-Local Storage).
class ScopedArenaTLS
{
public:
    explicit ScopedArenaTLS(const std::shared_ptr<Arena>& arena)
      : _prevArenaTLS(Arena::getArenaTLS())
    {
        Arena::setArenaTLS(arena);
    }

    ~ScopedArenaTLS()
    {
        Arena::setArenaTLS(_prevArenaTLS);
    }

private:
    std::shared_ptr<Arena> _prevArenaTLS;
};
...
class Allocator
{
 ...
 public:                   // Allocation
            template<class ... Args>
            void              construct(type* p, Args&&... args)               {::new(p) type(std::forward<Args>(args)...);}
...
/**
 * std::scoped_allocator_adaptor around an allocator,
 * typically used when defining an outer container that contains other containers.
 */
template<class T>
using ScopedAllocatorAdaptor = std::scoped_allocator_adaptor<Allocator<T>>;
...
{
 public:                   // Construction
            /// In most cases, the parent arena should be getArena().
            /// The only exception is in the implementation of getArena() itself:
            ///  - getArena() needs an Options object to construct an arena to return;
            ///  - The parent as specified in this Options object cannot be getArena (to void an infinite loop);
            ///  - So in this one case, we construct Options with getRootArena().
            explicit          Options(name_t n = "", const ArenaPtr& parent = getArena());
 ...
 private:                  // Implementation
            ...
            virtual int       getConfiguredArenaPageSize() const;
 ...
 *  instead taking on the responsibility of finalization manually:
 *  @code
