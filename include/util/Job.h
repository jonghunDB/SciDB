...
private:
    ...
    /// On stack object that pushes and pops a given job on/off the per-thread job stack
    class ThreadScopedPtr
    {
    public:
        ThreadScopedPtr(const std::shared_ptr<Job>& curJob)
        {
            Job::pushJobPerThread(curJob);
        }
        ~ThreadScopedPtr()
        {
            Job::popJobPerThread();
...
public:
    ...
    /// Pop the top job from the thread local stack
    /// @pre the stack cannot be empty
    static void popJobPerThread();

    /// Push a job into the thread local stack
    /// @note std::weak_ptr<Job>(job) is pushed on the stack
    static void pushJobPerThread(const std::shared_ptr<Job>& job);
