...
/**
 * Use the C++ abi libary to demangle a c++ type into "human readable" form.
 *
 * @param name the c++ string to demangle
 * @return the demangled c++ name if the input is actually a c++ mangled name, else the
 *         name is returned unchanged
 */
std::string demangle_cpp(const char* name);


/**
 * Get the underlying type of a strongly type enum (c++14). See Effective Modern C++ by
 * Scott Meyers, Item 10, page 73.
 *
 * Example:
 * @code {.cpp}
 * enum Color : uint32_t {
 ...
 * std::cout << toUType(myColor) << std::endl;
 * @endcode
 * This will output "1",
 */
template <typename E>
