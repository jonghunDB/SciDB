...
class MpiLauncher : public std::enable_shared_from_this<MpiLauncher>
{
 public:
    ...
    public:
                        ...
                        "SCIDB_SE_INTERNAL", "SCIDB_LE_UNKNOWN_ERROR", INVALID_QUERY_ID)
      {
       ...
       {
           std::shared_ptr<InvalidStateException> ep =
                   std::make_shared<InvalidStateException>(_file.c_str(),
                                                           _function.c_str(),
                                                           _line);
 ...
 protected:
    ...
    static const InstanceDesc* getSortedInstances(std::vector<const InstanceDesc*>& sortedInstances,
