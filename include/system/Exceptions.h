...
#   define ASSERT_EXCEPTION(_cond_, _msg_)                              \
    do {                                                                \
        bool cond = static_cast<bool>(_cond_);                          \
        if (!cond) {                                                    \
            assert(cond);                                               \
...
/**
 * The macro is used to declare a subclass of SystemException which hard-codes
 * the short and long error codes.  The specific subclass can then be the
 * target of a catch statement.
 */
#define DECLARE_SYSTEM_EXCEPTION_SUBCLASS(exception_name, short_error_code, long_error_code) \
    class exception_name : public scidb::SystemException \
    { \
    public: \
        exception_name(const char* file, \
                       const char* function, \
                       int32_t line) \
        : SystemException(file, \
                          function, \
                          line, \
                          "scidb", \
                          short_error_code, \
       ...
       virtual Exception::Pointer copy() const \
        { \
            std::shared_ptr<exception_name> ep = \
                std::make_shared<exception_name>(_file.c_str(), \
                                                 _function.c_str(), \
                                                 _line); \
...
#define SYSTEM_EXCEPTION_SUBCLASS(exception_name) \
    exception_name(REL_FILE, __FUNCTION__, __LINE__)
...
public:
    ...
    // Make sure to avoid XxxException object slicing by calling raise().
