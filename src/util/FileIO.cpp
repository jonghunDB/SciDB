...
namespace scidb
{
    ...
    File::readDir(const char* dirName, std::list<std::string>& entries)
    {
        ...
        boost::function<void (struct dirent* de)> cb =
            boost::bind(&File::readDirCb, entries, _1);
    ...
    int File::fallocPunchHole(off_t offset, off_t len)
    {
        ...
        if(isDebug()) { // avoid cost of fstat() for production builds
...
{
    // called more than 1M times/sec from load() operator for single char reads
    // requires statistically sampled wait timing to keep the timing load at a
    // fraction of the read cost when size*nmemb is small.
    static thread_local uint64_t unsampledCount=0;      // per-thread sub-sampling memo
    const uint64_t SAMPLE_INTERVAL=101;  // prime avoids some aliasing
                                     // >100 drops cost sufficiently for this case
    ScopedWaitTimer timer(WaitTimerParams(PTW_SWT_FS_RD, SAMPLE_INTERVAL, &unsampledCount));
...
{
    // called at high rates from save() operators with short writes
    // requires statistically sampled wait timing to keep the timing load at a
    // fraction of the read cost when size*nmemb is small.
    static thread_local uint64_t unsampledCount=0;      // per-thread sub-sampling memo
    const uint64_t SAMPLE_INTERVAL=101;  // prime avoids some aliasing
                                     // >100 drops cost sufficiently for this case
    ScopedWaitTimer timer(WaitTimerParams(PTW_SWT_FS_WR, SAMPLE_INTERVAL, &unsampledCount));
