...
void Translator::passSchema(const Node* ast,ArrayDesc& schema,const string& arrayName)
{
    ...
    attributes.push_back(
        AttributeDesc(
            ...
            TID_INDICATOR, AttributeDesc::IS_EMPTY_INDICATOR, CompressorType::NONE));
...
LQPNPtr Translator::passAFLOperator(const Node *ast, size_t depthOperator)
{
    ...
    // Get refs to positional and keyword arguments, and count the number of positionals given.
    const Placeholders& posPlaceholders = op->getParamPlaceholders();
    const KeywordPlaceholders& kwPlaceholders = op->getKeywordPlaceholders();
    size_t numPositionals = 0;
    for (Node* const astNode : astParameters) {
        if (!astNode->is(kwarg)) {
            numPositionals += 1;
        }
    }
    ...
    if (posPlaceholders.empty() && kwPlaceholders.empty())
    {
    ...
    {
        // If operator positional parameters are variable, we don't know the exact count of needed parameters.
        // If not variable, check AST's positional parameters count against positional placeholders count.
        const bool hasVaryParams = posPlaceholders.back()->getPlaceholderType() == PLACEHOLDER_VARIES;
        const bool tooFewParams = hasVaryParams
            ? numPositionals + 1 < posPlaceholders.size() // +1 for PLACEHOLDER_VARIES
            : numPositionals != posPlaceholders.size();
        if (tooFewParams)
        {
            size_t expected = posPlaceholders.size() - size_t(hasVaryParams);
            fail(SYNTAX(SCIDB_LE_WRONG_OPERATOR_ARGUMENTS_COUNT, ast->get(applicationArgOperands))
                 << opName << expected << numPositionals);
        }

        // Iterate over all positional parameters of operator and match placeholders
        Placeholders expectedPlaceholders;
        size_t astParamNo = 0;
        for ( ; astParamNo < numPositionals; ++astParamNo)
        {
            //First we iterating over all fixed parameters, and then over all vary parameters
            if (hasVaryParams && astParamNo >= posPlaceholders.size() - 1)
            {
                expectedPlaceholders = op->nextVaryParamPlaceholder(inputSchemas);
                if (expectedPlaceholders.size() == 1 &&
                    placeholdersVectorContainType(expectedPlaceholders, PLACEHOLDER_END_OF_VARIES))
                {
                    // No more optional args wanted, yet I still have a positional argument here!
                    fail(SYNTAX(SCIDB_LE_WRONG_OPERATOR_ARGUMENTS_COUNT3, astParameters[astParamNo])
                         << opName << astParamNo);
                }
            }
            else
            {
                // Still working on the non-varying positionals.
                expectedPlaceholders.clear();
                expectedPlaceholders.push_back(posPlaceholders[astParamNo]);
            }

            // Handle positional parameter.
            Node *astParam = astParameters[astParamNo];
            SCIDB_ASSERT(!astParam->is(kwarg));
            std::shared_ptr<OperatorParam> opParam;
            if (matchPositionalParam(astParam, expectedPlaceholders, inputSchemas, opInputs,opParam,depthOperator))
            {
                op->addParameter(opParam);
            }
        }

        // Done with the supplied positional arguments.
        if (hasVaryParams)
        {
            // Grab the next set of placeholders and make sure that end-of-varies is among them,
            // that is, make sure that operator is not requiring any additional positional argument.
            expectedPlaceholders = op->nextVaryParamPlaceholder(inputSchemas);
            if (!placeholdersVectorContainType(expectedPlaceholders, PLACEHOLDER_END_OF_VARIES))
            {
                fail(SYNTAX(SCIDB_LE_WRONG_OPERATOR_ARGUMENTS_COUNT2,ast->get(applicationArgOperands)) << opName);
            }
        }

        // Have keyword parameters but operator says it wants none?  Bad!
        if (astParamNo != astParameters.size() && kwPlaceholders.empty())
        {
            fail(SYNTAX(SCIDB_LE_OPERATOR_FORBIDS_KEYWORD_PARAMS,ast->get(applicationArgOperands)) << opName);
        }

        // Handle keyword parameter(s).
        for (; astParamNo < astParameters.size(); ++astParamNo)
        {
            Node *astParam = astParameters[astParamNo];
            SCIDB_ASSERT(astParam->is(kwarg));
            char const* kw = astParam->get(kwargArgKeyword)->getString();
            Node* astArg = astParam->get(kwargArgArgument);
            Parameter opParam =
                matchKeywordParam(kw, astArg, kwPlaceholders, inputSchemas, opInputs, depthOperator);
            ASSERT_EXCEPTION(opParam.get(), "matchKeywordParam() did not throw yet returned nothing");
            op->addParameter(kw, opParam);
        }
    }
    ...
    for (Parameter const& param : op->getParameters())
    {
        if (PARAM_LOGICAL_EXPRESSION != param->getParamType())
            continue;
        checkParamExpression(param, result, inputSchemas);
    }
    for (auto const& kwArgPair : op->getKeywordParameters())
    {
        Parameter const& param = kwArgPair.second;
        if (PARAM_LOGICAL_EXPRESSION != param->getParamType())
            continue;
        checkParamExpression(param, result, inputSchemas);
    }
                                      ...
                                      vector<ArrayDesc> const& inputSchemas)
{
    ...
    const ArrayDesc& outputSchema = lqpn->inferTypes(_qry);
...
Translator::createArrayReferenceParam(const Node *arrayReferenceAST, bool inputSchema)
{
    ...
    SCIDB_ASSERT(!ArrayDesc::isNameVersioned(arrayName));
...
Translator::createNamespaceReferenceParam(const Node *nsRefAst, bool inputSchema)
{
    ...
    assert(ArrayDesc::isNameUnversioned(nsName));
    ...
    return make_shared<OperatorParamNamespaceReference>(newParsingContext(nsRefAst), nsDesc);
}

bool Translator::matchPositionalParam(const Node* ast,
            ...
            case PLACEHOLDER_ATTRIBUTE_NAME:
            {
                if (ast->is(reference) && !ast->has(referenceArgVersion))
                {
                    ...
                    std::shared_ptr<OperatorParamAttributeReference> opParam =
                        make_shared<OperatorParamAttributeReference>(
            ...
            case PLACEHOLDER_DIMENSION_NAME:
            {
                if (ast->is(reference) && !ast->has(referenceArgVersion))
                {
                    ...
                    std::shared_ptr<OperatorParamReference> opParam =
                        make_shared<OperatorParamDimensionReference>(
                            newParsingContext(ast),
                            getStringVariableName(ary),
                            getStringVariableName(var),
                            placeholder->isInputSchema());
...
/**
 * @brief Given keyword and argument, validate the argument type and return the OperatorParam pointer.
 *
 * @description
 * Implemented as a wrapper around matchPositionalParam() so that we get the same semantics for
 * both positional and keyword arguments.
 *
 * @param keyword the keyword
 * @param astArg AST node of the keyword argument's value
 * @param kwHolders operator-provided specification of valid types for this argument
 * @param inputSchemas schemas of inputs seen so far
 * @param inputs logical plan nodes of inputs
 * @param depth depth of the current operator in the logical query tree
 * @return pointer to base OperatorParam with correct derived type
 *
 * @note We allow a particular keyword to take arguments of any of several types, but that may be
 *       overkill.  Not used anywhere yet, but it might be useful.  If an individual operator wants
 *       to use this capability, it will have to check the OperatorParam::getParamType() of whatever
 *       it receives.
 */
Parameter Translator::matchKeywordParam(const char* keyword,
                                        Node const* astArg,
                                        KeywordPlaceholders const& kwHolders,
                                        vector<ArrayDesc>& inputSchemas,
                                        vector<LQPNPtr>& inputs,
                                        size_t depth)
{
    // Gather permitted placeholders (that is, argument types).
    Placeholders holders;
    KeywordPlaceholders::const_iterator pos;
    for (pos = kwHolders.lower_bound(keyword);
         pos != kwHolders.upper_bound(keyword);
         ++pos)
    {
        holders.push_back(pos->second);
    }
    if (holders.empty())
    {
        fail(SYNTAX(SCIDB_LE_UNRECOGNIZED_KEYWORD_PARAM, astArg) << keyword);
    }

    // Pretend we have a positional parameter.
    Parameter result;
    if (!matchPositionalParam(astArg, holders, inputSchemas, inputs, result, depth))
    {
        // This could only happen if an array input was given.  Not allowed!
        fail(INTERNAL(SCIDB_LE_ILLEGAL_ARRAY_AS_KEYWORD_PARAM, astArg));
    }

    SCIDB_ASSERT(result.get());
    return result;
}
                                                ...
                                                bool throwException)
{
    ...
    for (ArrayDesc const& schema : inputSchemas)
    {
        ...
        for (AttributeDesc const& attribute : schema.getAttributes())
        {
...
LQPNPtr Translator::passJoinItem(const Node *ast, size_t depthOperator)
{
    ...
    switch (ast->getType())
    {
        case namedExpr:
        {
            ...
            result->inferTypes(_qry, true); // force update flag required after modifying the AliasName
...
LQPNPtr Translator::passIntoClause(const Node* ast, LQPNPtr &input, size_t depthOperator)
{
    ...
    Parameters targetParams(1,make_shared<OperatorParamArrayReference>(parsingContext,"",targetName,true));
    ...
    std::string qualName(ArrayDesc::makeQualifiedArrayName(nsName, targetName));
    ...
    else
    {
        ...
        try
        {
            ...
            redimOp->setParameters(Parameters(1,make_shared<OperatorParamSchema>(parsingContext, destinationSchema)));
...
LQPNPtr Translator::passUpdateStatement(const Node *ast, size_t depthOperator)
{
    ...
    for (const Node* updateItem : updateList->getList())
    {
        ...
        bool found = false;
        for (AttributeDesc const& att : arrayDesc.getAttributes())
        {
...
LQPNPtr Translator::passInsertIntoStatement(const Node *ast, size_t depthOperator)
{
    ...
    if (!found) {
        fail(QPROC(SCIDB_LE_ARRAY_DOESNT_EXIST, dstAst)
             << ArrayDesc::makeQualifiedArrayName(args.nsName, dstName));
    }
...
void Translator::checkLogicalExpression(const vector<ArrayDesc> &inputSchemas, const ArrayDesc &outputSchema,const LEPtr &expr)
{
    ...
    else if (typeid(*expr) == typeid(Function))
    {
        ...
        {
            LOG4CXX_DEBUG(logger, "checkLogicalExpression calling checkLogicalExprssion");
    ...
    size_t depthOperator)
{
        ...
        switch (selItem->getType())
        {
            case namedExpr:
            {
                 ...
                 && !(grwAsClause && grwAsClause->is(redimensionClause)))
                {
                    ...
                    for (AttributeDesc const& aDesc : inputSchema.getAttributes())
                    {
                        LOG4CXX_TRACE(logger, "Item is named expression");
            ...
            case asterisk:
            {
                ...
                for (AttributeDesc const& att : inputSchema.getAttributes(true))
                {
                    std::shared_ptr<OperatorParamReference> param = make_shared<OperatorParamAttributeReference>(
                        newParsingContext(selItem),
                        "",
    ...
    if (aggregateFunctions.size() > 0)
    {
        ...
        map<string, pair<string, Parameters> > aggregateParams;
            ...
            switch (grwAsClause->getType())
            {
                case list:
                {
                    ...
                    for (const Node* windowClause : grwAsClause->getList())
                    {
                        ...
                        for (pairOfNodes& wsize : windowSizes)
                        {
                ...
                case redimensionClause:
                {
                    ...
                    for (Node *aggCallNode : aggregateFunctions)
                    {
                        ...
                        if (aggCallNode->get(applicationArgOperands)->get(listArg0)->is(reference))
                        {
                            ...
                            for (AttributeDesc const& attr : preEvalInputSchemas[0].getAttributes())
                            {
        ...
        map<string,pair<string,Parameters> >::const_iterator it = aggregateParams.begin();
...
string Translator::genUniqueObjectName(const string& prefix, unsigned int &initialCounter,const vector<ArrayDesc> &inputSchemas, bool internal, cnodes namedExpressions)
{
    ...
    while(true)
    {
        ...
        for (ArrayDesc const& schema : inputSchemas)
        {
            for (AttributeDesc const& att : schema.getAttributes())
            {
                if (att.getName() == name)
                    goto nextName;
            }
...
LQPNPtr Translator::canonicalizeTypes(const LQPNPtr &input)
{
    ...
    const ArrayDesc& inputSchema = input->inferTypes(_qry);
    bool skip = true;
    for (AttributeDesc const& att : inputSchema.getAttributes())
    {
