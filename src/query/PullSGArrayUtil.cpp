...
SerializedArray::nextChunk(AttributeID attId, MemChunk& chunk)
{
    ...
    if (_inputArray->getArrayDesc().getEmptyBitmapAttribute()
        && ((_inputArray->getArrayDesc().getAttributes().size() - 1) == currInputAttr))
    {
...
SerializedArray::getSingleAttributeDesc(const ArrayDesc& desc)
{
    ...
    outAttributes.push_back(AttributeDesc(0,SERIALIZED_ATTR_NAME,TID_VOID,0,CompressorType::NONE));
...
{
    const AttributeID currAttr = *_currAttribute;

    if (attId != currAttr) {
        LOG4CXX_ERROR(PullSGArray::_logger, "DeserializedArray::"<<__func__<< " got attId " << attId << " but expected cur= " << currAttr);
    }
    ASSERT_EXCEPTION(attId == currAttr, "DeserializedArray::nextChunk(): Redistributed chunk consumed out of order");

    if (!_inputIterator) {
        _inputIterator = _inputArray->getConstIterator(AttributeID(0)); // start with first attribute
    } else {
        ++(*_inputIterator);
    }
                   ...
                   bool enforceDataIntegrity)
{
    ...
    if (access == Array::SINGLE_PASS) {
        ...
        if (singlePass) {
            ...
            ASSERT_EXCEPTION(singlePass, __func__ );
            // used to assert if not getting a "singlePassArray" here...
            // probably because the others can block,even Deserialized Array
            DeserializedArray* deserial = dynamic_cast<DeserializedArray*>(inputArray.get());
            ASSERT_EXCEPTION(deserial, __func__ );
        }
    }
