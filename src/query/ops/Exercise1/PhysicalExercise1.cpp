...
namespace scidb {
    ...
    public:
        ...
        std::shared_ptr<Array> execute(std::vector< std::shared_ptr<Array> >& inputArrays, std::shared_ptr<Query> query) override
        {
            ...
            outputAttr.push_back(AttributeDesc( attributeID, "attr1", TID_DOUBLE, 0, CompressorType::NONE));
            ...
            Coordinates startingCell = getWindowStart(inDesc);;
            Coordinates endingCell= getWindowEnd(inDesc);
...
/*            LOG4CXX_DEBUG(logger,"make startingcell");
            ...
            LOG4CXX_DEBUG(logger,"input array Iterator start");
            while (!arrayIterator->end())
            {
                ...
                while(!chunkIterator->end()) {
                    ...
                    for(size_t i =0 ; i <nDims; i++)
                    {
                        if(chunkIterator->getPosition()[i] > endingCell[i])
                        {
                            checkboundary &= false;
                        }
                    }
        ...
        std::shared_ptr<Array> Exercise1Array (Coordinates startingCell,Coordinates endingCell,vector<double> attributeValues,std::shared_ptr<Query>& query)
        {
            ...
            while(!outputArrayIter->end()){
                while(!outputChunkIter->end())
                {
                    ...
                    LOG4CXX_INFO(logger,"attributesvalues size "<< attributeValues.size());

                    Coordinates co = outputChunkIter->getPosition();
                    int64_t index =0;
                    for(size_t i =0 ; i <2 ;i++ )
                    {
                        ...
                        Value value;
                        value.setDouble(attributeValues[index]);
                        LOG4CXX_INFO(logger,"output value" << value.getUint32());
                        outputChunkIter->writeItem(value);

                        LOG4CXX_INFO(logger,"write value" );
