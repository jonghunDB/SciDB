...
public:
    ...
    ArrayDesc inferSchema(std::vector<ArrayDesc> schemas, std::shared_ptr<Query> query)
    {
        ...
        uint32_t numQuantilesPlusOne = 1 +  evaluate(
            ((std::shared_ptr<OperatorParamLogicalExpression>&)_parameters[0])->getExpression(), TID_UINT32).getUint32();
        ...
        outputAttrs.push_back(
            AttributeDesc(0, "percentage", TID_DOUBLE, 0,CompressorType::NONE));
        outputAttrs.push_back(
            AttributeDesc(1, attName + "_quantile", inputAttribute.getType(),
