        ...
        for (key, value) in config.items(section_name):
            ...
            if key.startswith('server-'):
                ...
                if srvId in srvIdSet:
                    ...
                    # Please forgive the late import but the correct sys.path doesn't occur reliably until this
                    # point.  We need to rethink our python strategy and make sure the appropriate PYTHONPATH
                    # is set for setting up, installing, running, and testing.
                    from scidb_config import ServerEntry

                    printDebug("Adding server-id=%d" % (srvId))
                    srvIdSet.add(srvId)
                    options['srvList'].append(ServerEntry(srvId, host, valueSplit[1:]))
...
def getOS(scidbEnv):
    # TODO: Use platform.dist() in python directly.
    # Example Output: ('Ubuntu', '14.04', 'trusty')
    #                 ('centos', '6.7', 'Final')
...
def runSetup(scidbEnv, force, sourcePath, buildPath, installPath, cmake_defs={}, name='scidb'):
    cmake_defs['SCIDB_SOURCE_DIR'] = cmake_defs.get('SCIDB_SOURCE_DIR', sourcePath)
...
def getInstances(configFile, dbName):
    configOpts = parseConfig(configFile, dbName)
    instances = 0
    for srv in configOpts['srvList']:
        instances = instances + len(srv._instances)
    return instances


def getNodes(configFile, dbName):
    configOpts = parseConfig(configFile, dbName)
    return len(configOpts['srvList'])
...
def install(scidbEnv):
    ...
    # TODO use the python provided platform module
    # if ''.join(platform.dist()[:2]).startswith('centos6')
    if platform.startswith("CentOS"):
        ...
        executeIt(cmdList, useShell=True)
    ...
    # Initialize SciDB
    scidbEnv.execute_scidb_py("initall-force")
...
def start(scidbEnv):
    scidbEnv.execute_scidb_py("startall")


def stop(scidbEnv):
    scidbEnv.execute_scidb_py("stopall")


def initall(scidbEnv):
    print "initializing cluster '{0}' from {1}".format(scidbEnv.args.scidb_name,
                                                       os.path.join(scidbEnv.install_path, "etc/config.ini"))
    scidbEnv.execute_scidb_py("initall")
...
def runTests(scidbEnv, testsPath, srcTestsPath, commands=None):
    ...
    disabledTests = os.path.join(testsPath, 'testcases', 'disable.tests')
    ...
    new_disabled_tests = '/tmp/disable.tests'
...
class SciDBEnv(object):
    ...
    def execute_scidb_py(self, command):
        authfile_arg = None
        if 'auth_file' in self.args and self.args.auth_file:
            authfile_arg = ["--auth-file", self.args.auth_file]
        cmdList = [self.scidb_py_path(),
                   command]
        if authfile_arg:
            cmdList += authfile_arg
        cmdList += [self.args.scidb_name,
...
def main(argv=None):
    ...
    # Please forgive the last PYTHONPATH addition.  We can't reliable import until the bin directory is
    # available during install.  scidb.py needs the scidb_config.py to pull stuff out of config.ini.
...
instance directories (aka base-path). Default is '$SCIDB_BUILD_PATH/DB-$SCIDB_NAME'\n
    ...
    install_parser.add_argument(
        ...
        default=None,
        nargs='?',
        help="config.ini file to use with scidb.py, default is generated")
    install_parser.add_argument(
        '--pg-user',
