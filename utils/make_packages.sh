...
Usage:
    ...
    chroot_distro - centos-6-x86_64 or ubuntu-trusty-amd64
EOF
    exit 1
}
...
case $distro in
    ...
    "ubuntu-trusty-amd64");;
    *)
        error "Invalid distro '${distro}'. Should be one of 'centos-6-x86_64' or 'centos-7-x86_64' or 'ubuntu-trusty-amd64'."
        ;;
esac
...
{
  if [ $target != "insource" ]; then
      info Removing build_dir = ${build_dir}
      rm -rf "${build_dir}"
...
if [ "$type" == "deb" ]; then
    ...
    function deb_prepare_sources ()
    {
        ...
        for filename in changelog rules conffiles; do
    ...
    if [ $target != "insource" ]; then
        ...
        pushd ${build_dir}/.. > /dev/null 2>&1
           info Moving result from ${build_dir}/.. to ${result_dir}
    ...
    if [ $target != "insource" ]; then
        ...
        if [ "$target" == "local" ]; then
            ...
            popd > /dev/null 2>&1
            info Moving result from "${build_dir}"/SRPMS and "${build_dir}"/RPMS and to ${result_dir}
