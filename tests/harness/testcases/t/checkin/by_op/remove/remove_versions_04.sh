...
for POWER2 in  2 16 128 512 4096 32768 ; do
  for DELTA in -1 0 1 ; do

    ((CHUNK_SIZE = POWER2 + DELTA))

    # create the array
    $IQUERY -naq "create array foo <a:int8> [I=1:16*${CHUNK_SIZE}, ${CHUNK_SIZE}, 0]"
    if [[ $? != 0 ]] ; then cleanup 1 $LINENO; fi

    # version1: fill in a manner that is non-RLE-compressible
    $IQUERY -naq "store(build(foo,I%2),foo)"
    if [[ $? != 0 ]] ; then cleanup 1 $LINENO; fi

    # version2: delete all cells after the first chunk
    $IQUERY -naq "delete(foo, I > ${CHUNK_SIZE})"
    if [[ $? != 0 ]] ; then cleanup 1 $LINENO; fi

    # get uaid for datastore lookup
    uaid=`$IQUERY -ocsv -aq "project(filter(list('arrays'),name='foo'),uaid)"`
    #if [[ $? != 0 ]] ; then cleanup 1 $LINENO; fi

    #DEBUG echo "CHUNK_SIZE $CHUNK_SIZE before REMOVE VERSIONS @@@@@@@@@@@@@@@@@@" 1>&2

    #DEBUG $IQUERY -aq "project(filter(list('datastores'), uaid=$uaid), file_bytes, file_free_bytes)" 1>&2
