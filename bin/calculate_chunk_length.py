...
Version history:
  - 16.9: use scidblib.scidb_schema parser and new dimension syntax.
...
def dbg(*args):
    if _verbose > 1:
        print >>sys.stderr, "DBG:", ' '.join(str(x) for x in args)
...
class Dimension:
    ...
    def to_tuple(self):
        """Reconstruct scidb_schema.Dimension namedtuple object from self."""
        return scidb_schema.Dimension(self.dim_name,
                                      self.dim_low, self.dim_high,
...
def raise_if_duplicates(it, description):
    ...
    if len(unique) < len(it):
        ...
        raise scidblib.AppError(
            'Error! There are multiple occurrences of the same {0}: {1}'.format(
                name, it2))
...
def parse_dimensions(s):
    ...
    return [Dimension(x.name, x.lo, x.hi, x.chunk, x.overlap) for x in raw_dims]
...
class NamesInLoadArray:
    ...
    def __init__(self, iquery_cmd, load_array):
        ...
        # attributes
        self.attrs = parse_attributes(str_attrs)
        for i, attr in enumerate(self.attrs):
            one_name = NameInLoadArray(attr.name,
                                       is_dim = False,
                                       is_int64 = attr.type=='int64',
                                       local_index = i)
            self.list.append(one_name)

        # dimensions
        self.dims = parse_dimensions(str_dims)
