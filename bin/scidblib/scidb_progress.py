...
class VersionAndDate:
    ...
    @example the last modification version/date of a script may be '16.6 (2016-4-13)'
    ...
    Details of public attributes:
      ...
      - day:      day
    """
    re_whole = r'\b\d+\.\d+\s\(\d+-\d+-\d+\)'

    def __init__(self, s):
        """Given a string, parse and set data members.

        @param s  a string of form '16.6 (2016-4-13)'.
        """
        self.major = None
        self.minor = None
        self.year = None
        ...
        re_components = r'(\d+)\.(\d+)\s\((\d+)-(\d+)-(\d+)\)'
        match_components = re.match(re_components, s)
        if match_components:
            self.major = int(match_components.group(1))
            self.minor = int(match_components.group(2))
            self.year = int(match_components.group(3))
            self.month = int(match_components.group(4))
            self.day = int(match_components.group(5))

    def valid(self):
        """Whether the stored values constitute a valid VersionAndDate.

        @return whether all five components are non-negative integers and the date is valid.
        """
    ...
    def __str__(self):
        ...
        return '{0}.{1} ({2}-{3}-{4})'.format(self.major, self.minor, self.year,
                                                  self.month, self.day)
