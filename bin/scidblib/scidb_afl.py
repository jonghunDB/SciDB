...
####################################################################
# DEPRECATED!  Please use scidlib.iquery_client in new code.       #
# Other functionality here is covered by scidblib.util.make_table. #
####################################################################
...
def get_instances_info(iquery_cmd = None):
    ...
    args = HostPort(os.environ.get('IQUERY_HOST',None),
                    os.environ.get('IQUERY_PORT',None))
    if not iquery_cmd:
        iquery_cmd = get_iquery_cmd(args=args,base_iquery_cmd='iquery -o csv:l')
