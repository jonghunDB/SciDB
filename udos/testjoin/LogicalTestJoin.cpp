...
class LogicalTestJoin: public LogicalOperator
{
  public:
    ...
    //logicaloperator의 Key부분. output의 schema를 설정.
...
//        * @param flags array flags from ArrayDesc::ArrayFlags
        ...
        // 왼쪽 배열로 부터 empty cell은 빼고 joinAttributes[j]에 정보를 저장함 left array의 attribute의 개수와 정보를 저장.
        ...
        if (leftArrayDesc.isAutochunked()) {
            ...
            // schema (index 1)
            //왼쪽 배열이 autochunked이면 오른쪽이 exemplar
            exemplarIndex = 1;
            targetIndex = 0;
        }
        ...
        {
            //ostringstream: 어떤 뎅터를 계속 메모리 스트림에 넣었다가 나중에 한번에 사용할때 보통 씀.
        ...
        // 두 배열의 dimension에서 시작하는 지점이 같은지 확인한다.
...
REGISTER_LOGICAL_OPERATOR_FACTORY(LogicalTestJoin, "test_join");
//DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalTestJoin, "test_join")
